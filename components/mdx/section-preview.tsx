...
export function SectionPreview({
    ...
    const n = Index[config.style][name]?.name.split("-");

    const a = Index[config.style][name]?.auth;
    const p = Index[config.style][name]?.pro;
            ...
            <div className={`relative mx-auto mt-32 first:mt-0`}>
                <div className="relative flex flex-col items-start justify-between md:flex-row md:items-center">
                    {/* <div>
                        <h2 className="flex items-end gap-2 truncate py-1 font-ins text-4xl font-normal capitalize tracking-tight text-blue-600 md:text-5xl">
                            ...
                            <Link
                                href={`/u/preview?component=${Index[config.style][name]?.name}`}
                                target="_blank"
                            >
                    ...
                    <div className="ml-auto flex items-center gap-10 md:-mt-20">
                        <div className="font-normal">
                            <div className="mx-auto flex w-fit items-center justify-center">
                                ...
                                {status === "authenticated" || a === "false" ? (
                                    <>
                                        {pro === true || p === "false" ? (
                                            ...
                                            <button
                                                ...
                                                className={`flex items-center rounded-md py-[0.45rem] pl-2 pr-2 text-sm tracking-wide lg:pr-3 ${activeTab === "code" ? "shadow" : ""} transition-all duration-500 ease-in-out`}
                                            >
                                                <Code className="size-5 text-neutral-700 dark:text-dawn-300" />

                                                <span className="sr-only text-neutral-700 dark:text-dawn-300 lg:not-sr-only lg:ml-2">
                                    ...
                                    // <Link
                                    //     href={`/login?callbackUrl=${Index[config.style][name]?.slug}`}
                                    ...
                                    <Dialog>
                                        ...
                                        <DialogContent className="py-20">
                                            <div className="mb-2 flex flex-col items-center gap-2">
                                                ...
                                                >
                                                    ...
                                                    <Image
                                                        ...
                                                        width={3840}
                                                        height={2160}
                                                        className="w-[2.75rem] dark:bg-white rounded-full"
                                                    />
                                                </div>
                                                <DialogHeader>
                                            ...
                                            >
                                                {loading && (
                                                    <Loader className="-ml-1 mr-3 h-5 w-5 animate-spin text-black" />
                                                )}
                                                <Image
                                                    src={GoogleLogo}
                                                    alt="google-logo"
                        ...
                        <Link
                            href={`/preview/${Index[config.style][name]?.name.split('-')[0]}/${Index[config.style][name]?.name}`}
                            target="_blank"
                        >
                ...
                >
                    <div className="overflow- col-span-2 row-start-2 mx-auto mt-8 min-w-0">
                        {activeTab === "preview" ? (
                            <div
                                className={`overflow-y-auto rounded-xl border font-ins dark:border-dawn-600 ${className} bg-white`}
                            >
                                {Preview}
                            </div>
                        ) : (
                            <div className="overflow-y-auto break-words rounded-xl text-sm">
